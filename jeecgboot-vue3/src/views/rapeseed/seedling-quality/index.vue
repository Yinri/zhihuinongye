<template>
  <div>
    ...
    <SeedlingQualityModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema } from './seedlingQuality.data';
  import SeedlingQualityModal from './SeedlingQualityModal.vue';
  import { 
    getSeedlingQualityList, 
    deleteSeedlingQuality, 
    exportSeedlingQuality, 
    importSeedlingQuality 
  ...
  const [registerTable, { reload, getSelectRows, dataSource }] = useTable({
    api: selectedPlotId.value ? (params) => {
      ...
      return getSeedlingQualityList(params);
    } : getSeedlingQualityList,
    columns,
    formConfig: {
      labelWidth: 120,
   ...
   * 加载秧苗质量数据
   */
  async function loadSeedlingQualityData() {
    if (!selectedBaseId.value) {
      return;
    }
    ...
    try {
      ...
      console.error('加载秧苗质量数据失败:', error);
      createMessage.error('加载秧苗质量数据失败，请稍后重试');
    } finally {
      loading.value = false;
    }
  }
  ...
  async function handleDelete(record: Recordable) {
    await deleteSeedlingQuality(record.id);
  ...
  async function handleExport() {
    const data = await getSeedlingQualityList(searchInfo);
    exportSeedlingQuality(data);
  }
  ...
  async function handleImport() {
    ...
    try {
      await importSeedlingQuality(formData);
