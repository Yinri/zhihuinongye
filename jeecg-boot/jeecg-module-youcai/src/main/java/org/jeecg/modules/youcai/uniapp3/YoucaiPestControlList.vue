...
layout: 'default',
style: {
navigationBarTitleText: '虫害防控表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="虫害防控表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './YoucaiPestControlData';
defineOptions({
  name: 'YoucaiPestControlList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/youcaiPestControl/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/youcaiPestControl/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
