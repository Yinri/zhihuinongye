...
<template>
  <PageLayout navTitle="地块信息表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './YoucaiPlotsData';
defineOptions({
  name: 'YoucaiPlotsList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/youcaiPlots/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/youcaiPlots/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
