...
<template>
  <PageLayout navTitle="灌溉监控表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './YoucaiIrrigationMonitoringData';
defineOptions({
  name: 'YoucaiIrrigationMonitoringList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/youcaiIrrigationMonitoring/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/youcaiIrrigationMonitoring/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
