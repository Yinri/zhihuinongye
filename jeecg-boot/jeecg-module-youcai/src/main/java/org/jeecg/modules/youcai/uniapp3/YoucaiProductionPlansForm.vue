      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['planYear']"
                  :label="get4Label('计划年份')"
                  name='planYear'
                  prop='planYear'
                  placeholder="请选择计划年份"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('油菜品种ID')"
                   name='varietyId'
                   prop='varietyId'
                   placeholder="请选择油菜品种ID"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入油菜品种ID!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['targetYield']"
                   :label="get4Label('目标产量(公斤/亩)')"
                   name='targetYield'
                   prop='targetYield'
                   placeholder="请选择目标产量(公斤/亩)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['plantingArea']"
                   :label="get4Label('种植面积(亩)')"
                   name='plantingArea'
                   prop='plantingArea'
                   placeholder="请选择种植面积(亩)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('计划播种日期')"
                  labelWidth="100px"
                  type="date"
                  name='plannedSowingDate'
                  v-model:value="myFormData['plannedSowingDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('计划收获日期')"
                  labelWidth="100px"
                  type="date"
                  name='plannedHarvestDate'
                  v-model:value="myFormData['plannedHarvestDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
...
const dataId = ref('')
const backRouteName = ref('YoucaiProductionPlansList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiProductionPlans/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiProductionPlans/edit':'/youcai/youcaiProductionPlans/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
