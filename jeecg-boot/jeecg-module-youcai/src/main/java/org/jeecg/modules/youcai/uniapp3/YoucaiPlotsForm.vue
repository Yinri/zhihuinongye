      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['baseId']"
                   :label="get4Label('所属基地ID')"
                   name='baseId'
                   prop='baseId'
                   placeholder="请选择所属基地ID"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入所属基地ID!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['area']"
                   :label="get4Label('地块面积(亩)')"
                   name='area'
                   prop='area'
                   placeholder="请选择地块面积(亩)"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入地块面积(亩)!'},
                   ]"
                   clearable
              />
        </view>
...
const dataId = ref('')
const backRouteName = ref('YoucaiPlotsList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiPlots/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiPlots/edit':'/youcai/youcaiPlots/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
