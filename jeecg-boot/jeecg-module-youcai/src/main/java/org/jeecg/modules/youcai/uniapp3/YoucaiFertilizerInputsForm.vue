        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('肥料类型')"
                  name='fertilizerType'
                  prop='fertilizerType'
                  placeholder="请选择肥料类型"
                  :rules="[
                                  { required: true, message: '请输入肥料类型!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fertilizerName']"
                  :label="get4Label('肥料名称')"
                  name='fertilizerName'
                  prop='fertilizerName'
                  placeholder="请选择肥料名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['npkRatio']"
                  :label="get4Label('NPK比例')"
                  name='npkRatio'
                  prop='npkRatio'
                  placeholder="请选择NPK比例"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('施肥量(公斤)')"
                   name='amount'
                   prop='amount'
                   placeholder="请选择施肥量(公斤)"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入施肥量(公斤)!'},
                   ]"
                   clearable
              />
        </view>
...
const dataId = ref('')
const backRouteName = ref('YoucaiFertilizerInputsList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiFertilizerInputs/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiFertilizerInputs/edit':'/youcai/youcaiFertilizerInputs/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
