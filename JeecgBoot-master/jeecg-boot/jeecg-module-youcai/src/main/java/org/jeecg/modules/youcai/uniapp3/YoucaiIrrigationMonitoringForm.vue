    ...
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('灌溉日期')"
                  labelWidth="100px"
                  type="date"
                  name='irrigationDate'
                  v-model:value="myFormData['irrigationDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['irrigationMethod']"
                  :label="get4Label('灌溉方式')"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['waterAmount']"
                   :label="get4Label('灌溉量(立方米)')"
                   name='waterAmount'
                   prop='waterAmount'
                   placeholder="请选择灌溉量(立方米)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['irrigationDuration']"
                   :label="get4Label('灌溉时长(分钟)')"
                   name='irrigationDuration'
                   prop='irrigationDuration'
                   placeholder="请选择灌溉时长(分钟)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
...
const dataId = ref('')
const backRouteName = ref('YoucaiIrrigationMonitoringList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiIrrigationMonitoring/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiIrrigationMonitoring/edit':'/youcai/youcaiIrrigationMonitoring/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
