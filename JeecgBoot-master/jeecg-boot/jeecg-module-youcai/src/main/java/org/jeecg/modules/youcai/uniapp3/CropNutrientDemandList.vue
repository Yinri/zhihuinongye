...
<template>
  <PageLayout navTitle="crop_nutrient_demand"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CropNutrientDemandData';
defineOptions({
  name: 'CropNutrientDemandList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/cropNutrientDemand/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/cropNutrientDemand/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
