      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('检测日期')"
                  labelWidth="100px"
                  type="date"
                  name='testDate'
                  v-model:value="myFormData['testDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['phValue']"
                   :label="get4Label('pH值')"
                   name='phValue'
                   prop='phValue'
                   placeholder="请选择pH值"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['organicMatter']"
                   :label="get4Label('有机质含量(g/kg)')"
                   name='organicMatter'
                   prop='organicMatter'
                   placeholder="请选择有机质含量(g/kg)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['nitrogen']"
                   :label="get4Label('氮含量(mg/kg)')"
                   name='nitrogen'
                   prop='nitrogen'
                   placeholder="请选择氮含量(mg/kg)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['phosphorus']"
                   :label="get4Label('磷含量(mg/kg)')"
                   name='phosphorus'
                   prop='phosphorus'
                   placeholder="请选择磷含量(mg/kg)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['potassium']"
                   :label="get4Label('钾含量(mg/kg)')"
                   name='potassium'
                   prop='potassium'
                   placeholder="请选择钾含量(mg/kg)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fertilityLevel']"
                  :label="get4Label('肥力等级')"
                  name='fertilityLevel'
                  prop='fertilityLevel'
                  placeholder="请选择肥力等级"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('YoucaiSoilFertilityList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiSoilFertility/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiSoilFertility/edit':'/youcai/youcaiSoilFertility/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
