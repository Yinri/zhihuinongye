    ...
    <scroll-view class="scrollArea" scroll-y>
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('播种日期')"
                  labelWidth="100px"
                  type="date"
                  name='sowingDate'
                  v-model:value="myFormData['sowingDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['sowingMethod']"
                  :label="get4Label('播种方式')"
                  name='sowingMethod'
                  prop='sowingMethod'
                  placeholder="请选择播种方式"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['seedingRate']"
                   :label="get4Label('播种量(公斤/亩)')"
                   name='seedingRate'
                   prop='seedingRate'
                   placeholder="请选择播种量(公斤/亩)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['actualSowingArea']"
                   :label="get4Label('实际播种面积(亩)')"
                   name='actualSowingArea'
                   prop='actualSowingArea'
                   placeholder="请选择实际播种面积(亩)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['sowingStatus']"
                  :label="get4Label('播种状态')"
                  name='sowingStatus'
                  prop='sowingStatus'
                  placeholder="请选择播种状态"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('YoucaiSowingMonitoringList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiSowingMonitoring/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiSowingMonitoring/edit':'/youcai/youcaiSowingMonitoring/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
