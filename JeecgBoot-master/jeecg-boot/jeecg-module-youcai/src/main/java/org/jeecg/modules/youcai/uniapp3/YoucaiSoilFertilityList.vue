...
<template>
  <PageLayout navTitle="土壤肥力表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './YoucaiSoilFertilityData';
defineOptions({
  name: 'YoucaiSoilFertilityList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/youcaiSoilFertility/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/youcaiSoilFertility/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
