      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['varietyName']"
                  :label="get4Label('品种名称')"
                  name='varietyName'
                  prop='varietyName'
                  placeholder="请选择品种名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('品种特性')"
                 ...
                 clearable
                 :maxlength="300"
                v-model="myFormData['characteristics']"
              ></wd-textarea>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['growthCycle']"
                   :label="get4Label('生长周期(天)')"
                   name='growthCycle'
                   prop='growthCycle'
                   placeholder="请选择生长周期(天)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['yieldPotential']"
                   :label="get4Label('产量潜力(公斤/亩)')"
                   name='yieldPotential'
                   prop='yieldPotential'
                   placeholder="请选择产量潜力(公斤/亩)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['diseaseResistance']"
                  :label="get4Label('抗病性')"
                  name='diseaseResistance'
                  prop='diseaseResistance'
                  placeholder="请选择抗病性"
                  :rules="[
                  ]"
                  clearable
...
const dataId = ref('')
const backRouteName = ref('YoucaiRapeVarietiesList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiRapeVarieties/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiRapeVarieties/edit':'/youcai/youcaiRapeVarieties/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
