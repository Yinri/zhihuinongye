        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('品种ID')"
                   name='varietyId'
                   prop='varietyId'
                   placeholder="请选择品种ID"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入品种ID!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['seedAmount']"
                   :label="get4Label('种子用量(公斤)')"
                   name='seedAmount'
                   prop='seedAmount'
                   placeholder="请选择种子用量(公斤)"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入种子用量(公斤)!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['seedCost']"
                   :label="get4Label('种子成本(元)')"
                   name='seedCost'
                   prop='seedCost'
                   placeholder="请选择种子成本(元)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
...
const dataId = ref('')
const backRouteName = ref('YoucaiSeedInputsList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiSeedInputs/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiSeedInputs/edit':'/youcai/youcaiSeedInputs/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
