...
navigationBarTitleText: 'variety_plot_utilization',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['varietyId']"
                  :label="get4Label('品种ID（关联作物品种表主键）')"
                  name='varietyId'
                  prop='varietyId'
                  placeholder="请选择品种ID（关联作物品种表主键）"
                  :rules="[
                                  { required: true, message: '请输入品种ID（关联作物品种表主键）!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('地块ID（关联地块表主键）')"
                  name='plotId'
                  prop='plotId'
                  placeholder="请选择地块ID（关联地块表主键）"
                  :rules="[
                                  { required: true, message: '请输入地块ID（关联地块表主键）!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fertilizerId']"
                  :label="get4Label('肥料ID（关联肥料表主键）')"
                  name='fertilizerId'
                  prop='fertilizerId'
                  placeholder="请选择肥料ID（关联肥料表主键）"
                  :rules="[
                                  { required: true, message: '请输入肥料ID（关联肥料表主键）!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['fertilizerType']"
                  :label="get4Label('肥料类型（冗余字段：如尿素、磷酸二铵）')"
                  name='fertilizerType'
                  prop='fertilizerType'
                  placeholder="请选择肥料类型（冗余字段：如尿素、磷酸二铵）"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['nRate']"
                   :label="get4Label('氮肥利用率(%)')"
                   name='nRate'
                   prop='nRate'
                   placeholder="请选择氮肥利用率(%)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['pRate']"
                   :label="get4Label('磷肥利用率(%)')"
                   name='pRate'
                   prop='pRate'
                   placeholder="请选择磷肥利用率(%)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['kRate']"
                   :label="get4Label('钾肥利用率(%)')"
                   name='kRate'
                   prop='kRate'
                   placeholder="请选择钾肥利用率(%)"
                   inputMode="numeric"
                   :rules="[
                   ]"
                   clearable
...
const dataId = ref('')
const backRouteName = ref('VarietyPlotUtilizationList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/varietyPlotUtilization/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/varietyPlotUtilization/edit':'/youcai/varietyPlotUtilization/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
