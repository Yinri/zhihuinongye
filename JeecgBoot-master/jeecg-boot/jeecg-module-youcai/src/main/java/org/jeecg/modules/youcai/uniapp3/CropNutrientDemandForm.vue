...
navigationBarTitleText: 'crop_nutrient_demand',
},
}
</route>
<template>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('作物品种ID（关联品种表）')"
                   name='varietyId'
                   prop='varietyId'
                   placeholder="请选择作物品种ID（关联品种表）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入作物品种ID（关联品种表）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['nDemand']"
                   :label="get4Label('N肥需肥量（kg/100kg产量）')"
                   name='nDemand'
                   prop='nDemand'
                   placeholder="请选择N肥需肥量（kg/100kg产量）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入N肥需肥量（kg/100kg产量）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['pDemand']"
                   :label="get4Label('P₂O₅需肥量（kg/100kg产量）')"
                   name='pDemand'
                   prop='pDemand'
                   placeholder="请选择P₂O₅需肥量（kg/100kg产量）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入P₂O₅需肥量（kg/100kg产量）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['kDemand']"
                   :label="get4Label('K₂O需肥量（kg/100kg产量）')"
                   name='kDemand'
                   prop='kDemand'
                   placeholder="请选择K₂O需肥量（kg/100kg产量）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入K₂O需肥量（kg/100kg产量）!'},
                   ]"
                   clearable
              />
        </view>
...
const dataId = ref('')
const backRouteName = ref('CropNutrientDemandList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/cropNutrientDemand/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/cropNutrientDemand/edit':'/youcai/cropNutrientDemand/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
