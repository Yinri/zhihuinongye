      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['baseName']"
                  :label="get4Label('基地名称')"
                  name='baseName'
                  prop='baseName'
                  placeholder="请选择基地名称"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('基地地址（具体地址）')"
                  name='address'
                  prop='address'
                  placeholder="请选择基地地址（具体地址）"
                  :rules="[
                                  { required: true, message: '请输入基地地址（具体地址）!'},
                  ]"
                  clearable
              />
        </view>
...
const dataId = ref('')
const backRouteName = ref('YoucaiBasesList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiBases/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiBases/edit':'/youcai/youcaiBases/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
