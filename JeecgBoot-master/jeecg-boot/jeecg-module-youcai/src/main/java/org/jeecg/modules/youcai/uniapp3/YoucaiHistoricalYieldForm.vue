...
navigationBarTitleText: 'youcai_historical_yield',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['year']"
                   :label="get4Label('年份（如2022）')"
                   name='year'
                   prop='year'
                   placeholder="请选择年份（如2022）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入年份（如2022）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['yield']"
                   :label="get4Label('单产（kg/亩）')"
                   name='yield'
                   prop='yield'
                   placeholder="请选择单产（kg/亩）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入单产（kg/亩）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   ...
                   :label="get4Label('品种ID（关联品种表）')"
                   name='varietyId'
                   prop='varietyId'
                   placeholder="请选择品种ID（关联品种表）"
                   inputMode="numeric"
                   :rules="[
                             { required: true, message: '请输入品种ID（关联品种表）!'},
                   ]"
                   clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['plot']"
                  :label="get4Label('地块名称')"
                  name='plot'
                  prop='plot'
                  placeholder="请选择地块名称"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('YoucaiHistoricalYieldList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiHistoricalYield/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiHistoricalYield/edit':'/youcai/youcaiHistoricalYield/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
