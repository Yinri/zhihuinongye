...
<template>
  <PageLayout navTitle="variety_plot_utilization"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './VarietyPlotUtilizationData';
defineOptions({
  name: 'VarietyPlotUtilizationList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/youcai/varietyPlotUtilization/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/youcai/varietyPlotUtilization/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
