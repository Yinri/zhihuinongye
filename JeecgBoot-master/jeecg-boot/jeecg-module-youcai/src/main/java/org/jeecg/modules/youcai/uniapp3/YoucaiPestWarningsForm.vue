      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['pestType']"
                  :label="get4Label('虫害类型')"
                  name='pestType'
                  prop='pestType'
                  placeholder="请选择虫害类型"
                  :rules="[
                                  { required: true, message: '请输入虫害类型!'},
                  ]"
                  clearable
              />
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['pestName']"
                  :label="get4Label('虫害名称')"
                  name='pestName'
                  prop='pestName'
                  placeholder="请选择虫害名称"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('YoucaiPestWarningsList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/youcai/youcaiPestWarnings/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/youcai/youcaiPestWarnings/edit':'/youcai/youcaiPestWarnings/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
