<template>
  <div class="irrigation-page">
    ...
    <a-card :bordered="false" v-if="selectedBaseId" class="table-card">
      ...
      <template #toolbar>
        <a-space>
          <a-button type="primary" preIcon="ant-design:plus-outlined" @click="handleCreate"> 新增灌溉记录</a-button>
      ...
      <BasicTable @register="registerTable" :searchInfo="searchInfo" :loading="loading">
        ...
        <!-- 空状态自定义 -->
        <template #emptyText>
          <a-empty description="该地块暂无灌溉记录">
            <template #image>
              <Icon icon="ant-design:cloud-outlined" style="font-size: 64px; color: #d9d9d9;" />
            </template>
    ...
    <a-card :bordered="false" v-if="selectedBaseId && selectedPlotId && !loading && dataSource.length === 0" class="empty-card">
      <a-result
        status="404"
        title="该地块暂无灌溉记录"
        sub-title="您可以点击下方按钮创建新的灌溉记录"
      >
        <template #icon>
          <Icon icon="ant-design:cloud-outlined" style="font-size: 64px; color: #d9d9d9;" />
        </template>
        <template #extra>
          <a-button type="primary" @click="handleCreate">创建灌溉记录</a-button>
        </template>
      </a-result>
    </a-card>
    ...
    <a-card :bordered="false" v-if="!selectedBaseId" class="empty-card">
      <a-result
        status="info"
        title="请先选择基地和地块"
        sub-title="选择基地和地块后即可查看和管理该地块的灌溉记录"
      >
        <template #icon>
    ...
    <!-- 表单弹窗 -->
    <IrrigationModal @register="registerModal" @success="handleSuccess" :baseId="selectedBaseId" :plotId="selectedPlotId" />
  </div>
</template>
...
import { columns, searchFormSchema } from './irrigation.data';
import { getIrrigationList, deleteIrrigation, exportIrrigation, importIrrigation } from './irrigation.api';
...
const [registerTable, { reload, getSelectRows, dataSource }] = useTable({
  ...
  api: selectedPlotId.value ? (params) => {
    ...
    return getIrrigationList(params);
  } : getIrrigationList,
  columns,
  formConfig: {
    labelWidth: 120,
...
async function loadIrrigationData() {
  ...
  try {
    ...
    console.error('加载灌溉数据失败:', error);
    createMessage.error('加载灌溉数据失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
...
async function handleDelete(record: Recordable) {
  await deleteIrrigation(record.id);
...
async function handleExport() {
  const data = await getIrrigationList(searchInfo);
  exportIrrigation(data);
}
...
async function handleImport() {
  ...
  try {
    await importIrrigation(formData);
