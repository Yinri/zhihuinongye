  ...
  <div class="fertilization-page">
    ...
    <a-card :bordered="false" v-if="selectedBaseId && selectedPlotId && !loading && dataSource.length === 0" class="empty-card">
      <a-result
        status="404"
        title="该地块暂无施肥记录"
        sub-title="您可以点击下方按钮创建新的施肥记录"
      >
        <template #icon>
          <Icon icon="ant-design:experiment-outlined" style="font-size: 64px; color: #d9d9d9;" />
        </template>
        <template #extra>
          <a-button type="primary" @click="handleCreate">创建施肥记录</a-button>
        </template>
      </a-result>
    </a-card>
    ...
    <a-card :bordered="false" v-if="!selectedBaseId" class="empty-card">
      <a-result
        status="info"
        title="请先选择基地和地块"
        sub-title="选择基地和地块后即可查看和管理该地块的施肥记录"
      >
        <template #icon>
    ...
    <!-- 表单弹窗 -->
    <FertilizationModal @register="registerModal" @success="handleSuccess" :baseId="selectedBaseId" :plotId="selectedPlotId" />
  </div>
</template>
...
const [registerTable, { reload, getSelectRows, dataSource }] = useTable({
  ...
  api: selectedPlotId.value ? (params) => {
    ...
    return getFertilizationList(params);
...
async function loadFertilizationData() {
  ...
  try {
    ...
    console.error('加载施肥数据失败:', error);
    createMessage.error('加载施肥数据失败，请稍后重试');
  } finally {
    loading.value = false;
  }
}
